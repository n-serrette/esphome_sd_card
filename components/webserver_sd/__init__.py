    ...
    cv.Schema(
        {
            ...
            cv.GenerateID(sd_card.CONF_SD_CARD_ID): cv.use_id(sd_card.SdCard),
            cv.Optional(CONF_URL_PREFIX, default="file"): cv.string_strict,
            cv.Optional(CONF_SD_PATH, default="/"): cv.string_strict,
...
async def to_code(config):
    ...
    sdmmc = await cg.get_variable(config[sd_card.CONF_SD_CARD_ID])
    cg.add(var.set_sd_card(sdmmc))
    cg.add(var.set_url_prefix(config[CONF_URL_PREFIX]))
    cg.add(var.set_sd_path(config[CONF_SD_PATH]))
